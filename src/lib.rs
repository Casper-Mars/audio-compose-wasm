...
pub async fn synthesize_audio(json_input: &str) -> Result<String, JsValue> {
    ...
    let download_tasks = segments.iter().map(|segment| {
        ...
        async move {
            let buffer = download_audio(&url).await
    ...
    for result in results {
        match result {
            ...
            Err(e) => {
                return Err(JsValue::from(JsError::new(&format!("Download error: {}", e))));
            }
        }
    }
    ...
    for segment in &mut segments {
        if let Some(buffer) = download_map.get(&segment.url) {
            ...
            return Err(JsValue::from(JsError::new(&format!("Missing buffer for {}", segment.url))));
        }
    }
